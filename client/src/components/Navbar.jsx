...
const Navbar = () => {
  ...
  const AvatarButton = () => (
    <div className="relative" ref={dropdownRef}>
      ...
      >
        {user.avatar ? (
          <img
            src={user.avatar}
            alt="avatar"
            className="w-8 h-8 rounded-xl object-cover ring-2 ring-indigo-500/40 hover:ring-indigo-500 transition-all"
          />
        ) : (
          <div className="w-8 h-8 rounded-xl bg-gradient-to-br from-violet-500 to-indigo-600 flex items-center justify-center text-white font-bold text-sm shadow-md">
            {initial}
          </div>
        )}
      </button>
      ...
      {dropdownOpen && (
        <div className="absolute right-0 top-full mt-2 w-44 rounded-2xl shadow-2xl border border-gray-200 dark:border-gray-600 bg-white dark:bg-gray-800 overflow-hidden z-[999]">
          ...
          <button
            ...
            className="w-full flex items-center gap-2 px-4 py-3 text-sm font-semibold text-gray-700 dark:text-gray-200 bg-white dark:bg-gray-800 hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors"
          >
            <User size={15} /> View Profile
          </button>
          <button
            onClick={() => { setDropdownOpen(false); handleLogout(); }}
            className="w-full flex items-center gap-2 px-4 py-3 text-sm font-semibold text-red-600 dark:text-red-400 bg-white dark:bg-gray-800 hover:bg-red-50 dark:hover:bg-red-900/20 transition-colors"
          >
  ...
  return (
    <nav className="w-full border-b border-gray-200/40 dark:border-white/5 bg-white/80 dark:bg-black/60 backdrop-blur-2xl z-50">
      <div className="max-w-7xl mx-auto px-4 sm:px-6">
        ...
        {/* ── MOBILE: all 4 items (3 links + avatar) in ONE row with equal spacing ── */}
